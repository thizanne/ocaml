...
Pattern matching::
The toplevel comment of link:../lambda/matching.ml[Matching] gives an
overview of how pattern matching is handled across the compiler: type
checking (link:typecore.ml[Typecore]), exhaustiveness and redundancy
analysis (link:parmatch.ml[Parmatch]) and compilation to the Lambda
IR (link:../lambda/matching.ml[Matching] itself).
